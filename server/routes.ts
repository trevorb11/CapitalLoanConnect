...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Proxy GHL form submission - submits data to a GHL native form on behalf of the user
  // This endpoint forwards form data to GHL's native form submission endpoint,
  // so a single user submission registers in both our system and GHL's form activity.
  app.post("/api/ghl-form-submit", async (req, res) => {
    try {
      const { formId, fullName, email, phone, businessName, captchaToken, additionalFields } = req.body;
      ...
      if (!locationId) {
        console.warn("[GHL FORM] GHL_LOCATION_ID not configured, skipping form submission");
        return res.json({ success: false, message: "GHL not configured" });
      }
      ...
      if (fullName) {
        ...
        formDataPayload.first_name = nameParts[0] || "";
        formDataPayload.last_name = nameParts.slice(1).join(" ") || "";
        formDataPayload.full_name = fullName;
      }
      if (email) formDataPayload.email = email;
      if (phone) formDataPayload.phone = phone.replace(/\D/g, "").length === 10 ? "+1" + phone.replace(/\D/g, "") : phone;
      if (businessName) formDataPayload.company_name = businessName;

      if (additionalFields && typeof additionalFields === "object") {
        Object.assign(formDataPayload, additionalFields);
      }

      const formBody = new FormData();
      formBody.append("formData", JSON.stringify(formDataPayload));
      if (captchaToken) {
        formBody.append("captchaV3", captchaToken);
      }

      console.log(`[GHL FORM] Submitting to form ${formId} for ${email}`);
      ...
      console.log(`[GHL FORM] Response status: ${ghlResponse.status}, body:`, JSON.stringify(responseData));

      if (ghlResponse.ok) {
        res.json({ success: true, data: responseData });
      } else {
        console.error(`[GHL FORM] Submission failed: ${ghlResponse.status}`, responseData);
        res.json({ success: false, status: ghlResponse.status, data: responseData });
      }
    } catch (error: any) {
      console.error("[GHL FORM] Error:", error);
