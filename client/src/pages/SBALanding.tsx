  ...
  useEffect(() => {
      ...
      try {
        if (Array.isArray(eventData) && eventData.length >= 3 && typeof eventData[2] === 'string') {
          ...
          if (jsonStr.includes('customer_id') || jsonStr.includes('full_address') || jsonStr.includes('email')) {
            ...
            console.log('[SBA GHL Form] Extracted from array format, keys:', Object.keys(parsed));
            return parsed;
          }
        }
        if (eventData && typeof eventData === 'object' && !Array.isArray(eventData)) {
          if (eventData.type === 'form-submit' || eventData.type === 'form-submit-success' || eventData.formId) {
            ...
            console.log('[SBA GHL Form] Extracted from object format, type:', eventData.type, 'keys:', Object.keys(data));
            return data;
          }
        }
        if (typeof eventData === 'string') {
          ...
          if (parsed && typeof parsed === 'object' && (parsed.email || parsed.full_name || parsed.phone)) {
            console.log('[SBA GHL Form] Extracted from string format, keys:', Object.keys(parsed));
            return parsed;
          }
        }
      } catch {
    ...
    const handleGhlMessage = (event: MessageEvent) => {
      ...
      try {
        ...
        if (!formData) return;
        console.log('[SBA GHL Form] Captured submission data:', JSON.stringify(formData, null, 2));
        console.log('[SBA GHL Form] All keys:', Object.keys(formData));
        ...
        if (!capturedBusiness) {
          ...
          if (businessKey) {
            ...
            console.log(`[SBA GHL Form] Found business name via key scan: "${businessKey}" = "${capturedBusiness}"`);
          }
        }
        console.log(`[SBA GHL Form] Captured business: "${capturedBusiness}", name: "${capturedName}", email: "${capturedEmail}"`);
